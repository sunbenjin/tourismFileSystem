...
(function (factory) {
    ...
    FileInput.prototype = {
        ...
        _initTemplateDefaults: function () {
            var self = this, tMain1, tMain2, tPreview, tFileIcon, tClose, tCaption, tBtnDefault, tBtnLink, tBtnBrowse,
                tModalMain, tModal, tProgress, tSize, tFooter, tActions, tActionDelete, tActionUpload, tActionDownload,
                 tActionDrag, tIndicator, tTagBef, tTagBef1, tTagBef2, tTagAft, tGeneric, tHtml, tImage,
                tText, tOffice, tGdocs, tVideo, tAudio, tFlash, tObject, tPdf, tOther, tStyle, tZoomCache, vDefaultDim,
                tStats,showDetail;/*tActionZoom,*/
            tMain1 = '{preview}\n' +
            ...
            tActionDelete = '<button type="button" class="kv-file-remove {removeClass}" ' +
                'title="{removeTitle}" {dataUrl}{dataKey}>删除</button>\n';
            tActionUpload = '<button type="button" class="kv-file-upload {uploadClass}" title="{uploadTitle}">' +
                '{uploadIcon}</button>';
            /*tActionDownload = '<a class="kv-file-download {downloadClass}" title="{downloadTitle}" ' +
                'href="{downloadUrl}" download="{caption}" target="_blank">{downloadIcon}</a>';*/
            tActionDownload = '<a class="kv-file-download {downloadClass}" title="{downloadTitle}" ' +
                'href="{downloadUrl}" download="{caption}" target="_blank">下载</a>';
           /* tActionZoom = '<button type="button"  class="kv-file-zoom {zoomClass}" ' +
                'title="{zoomTitle}">{zoomIcon}</button>';*/
           //  onclick="showPicDetail(\'{data}\')"'
            showDetail = '<button type="button"  class="kv-file-zoom {zoomClass}" property="{data}" ' +
                'title="{zoomTitle}">预览</button>';
            tActionDrag = '<span class="file-drag-handle {dragClass}" title="{dragTitle}"></span>';
           ...
           tImage = '<img src="{data}" class="file-preview-image kv-preview-data" title="{title}" ' +
                'alt="{alt}"' + tStyle + '>\n';
            /*tImage = '<iframe class="kv-preview-data file-preview-office" ' +
                /!*'src="http://1.0.7.107:8012/onlinePreview?url={data}"'*!/
                'src="http://127.0.0.1:8001/enclosure/fileInput/zoom?url={data}"'
                + tStyle + '></iframe>';*/
            /*  tImage='<iframe class="file-preview-image kv-preview-data" ' +
               'src="http://127.0.0.1:8001/enclosure/fileInput/zoom?url={data}"'
               + tStyle + '></iframe>';*/
            tText = '<textarea class="kv-preview-data file-preview-text" title="{caption}" readonly' + tStyle + '>' +
                '{data}</textarea>\n';
            tOffice = '<iframe class="kv-preview-data file-preview-office" ' +
                //'src="${priviewUrl}/onlinePreview?url={data}"'
                //'src="http://www.bj12301.cn:8012/onlinePreview?url={data}"'
                'src="http://127.0.0.1:8012/onlinePreview?url={data}"'
                 //'src="http://36.110.41.183:8012/onlinePreview?url={data}"'
               //  'src="http://39.97.230.101:8012/onlinePreview?url={data}"'
                + tStyle + '></iframe>';
           ...
           /* tObject = '<iframe id="urlImg" class="kv-preview-data file-preview-office " ' +
                'src="http://127.0.0.1:8001/enclosure/fileInput/zoom?url={data}"'
                + 'style="width: 100% !important;height: 100%;min-width:100% !important;max-width: 100% !important;min-height: 100% !important; border: 0px"' + '></iframe>';*/
            tOther = '<div class="kv-preview-data file-preview-other-frame"' + tStyle + '>\n' + $h.DEFAULT_PREVIEW + '\n</div>\n';
            tZoomCache = '<div class="kv-zoom-cache" style="display:none">{zoomContent}</div>';
            vDefaultDim = {width: '100% !imporant', height: '100%', 'min-height': '480px'};
        ...
        _zoomPreview: function ($btn) {
           ...
           var dataValue =  $btn.attr("property");
           var regex  = /.?(\.zip|\.rar|\.tar|\.gzip|\.gz|\.7z)$/g;
           if(dataValue.match(regex)){
               alert("压缩文件不支持预览！");
               return;
           }else {
               var imgReg =  /.?(\.jpg|\.jpeg|\.bmp|\.png)$/g;
               if(dataValue.match(imgReg)){
                   // window.open("http://36.110.41.183:18080/enclosure/fileInput/zoom?data="+dataValue);
                  //window.open("${basePath}/enclosure/fileInput/zoom?data="+dataValue);
                   window.open("http://127.0.0.1:8081/enclosure/fileInput/zoom?data="+dataValue);
                  // window.open("http://39.97.230.101:8001/enclosure/fileInput/zoom?data="+dataValue);
               }else{
                   var self = this, $frame, $modal = self.$modal;
                   self.getPreviewHeight("1000");//Resize the parent page window;
                   if (!$btn.length) {
